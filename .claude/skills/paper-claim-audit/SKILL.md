...
**CRITICAL: Use `codex exec` (new thread), NEVER `codex exec resume`.** Every run must be a fresh context.
...
- **Fresh thread EVERY run.** Never use `codex exec resume`. Never carry context.
...
`codex exec resume`. Do not accept prior audit outputs (PROOF_AUDIT, CITATION_AUDIT,
