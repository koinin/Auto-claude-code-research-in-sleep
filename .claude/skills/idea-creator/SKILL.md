...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via Codex CLI for brainstorming and review. Must be an OpenAI model (e.g., `gpt-5.5`, `o3`, `gpt-4o`).
...
Save the thread_id for follow-up.
...
2. **Critical review**: Use GPT-5.5 via `codex exec resume` (same thread):
   ```
