...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via Codex CLI for paper review.
- **REVIEWER_BIAS_GUARD = true** — When `true`, every review round uses a fresh `codex exec` thread with no prior review context. Never use `codex exec resume` for review rounds. Set to `false` only for deliberate debugging of the legacy behavior. **Empirical evidence (April 2026):** running the same paper with `codex exec resume` + "since last round we did X" prompts inflated scores from real 3/10 → fake 8/10 across 5 rounds; switching to fresh threads recovered the true 3/10 assessment.
...
- Never pass a prior thread_id into the next review prompt.
...
- If recovery metadata is needed, store the returned thread_id for crash recovery only; do not use it to preserve review context.
...
```bash
codex exec -m gpt-5.5 -c model_reasoning_effort="xhigh" "$(cat <<'PROMPT'
...
Save the thread_id for Round 2.
...
If `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `codex exec` thread for Round 2. Do not reuse the Round 1 thread_id for prompting. Save the returned thread_id only for recovery bookkeeping.
...
If `REVIEWER_BIAS_GUARD = false` (legacy debugging only), use `codex exec resume` with the saved thread_id; this is **not** the recommended path.
...
This is a late-stage adversarial check. It must always use **fresh** `codex exec` threads, never `codex exec resume`, and it must not reuse any prior review context.
