...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via Codex CLI for proposal review. Must be an OpenAI model (e.g., `gpt-5.5`, `o3`, `gpt-4o`).
...
**State**: Write `GRANT_STATE.json` with `phase: 2`, aims summary, and Codex thread_id.
...
> ⚠️ **Codex CLI fallback**: If `codex exec` is not available (no OpenAI API key), skip external review. Note "External review skipped — no Codex CLI available. Consider running `/auto-review-loop` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.
...
```bash
codex exec resume [from Phase 2] -c model_reasoning_effort="xhigh" "$(cat <<'PROMPT'
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via `codex exec resume`.
...
| `reviewer model` | gpt-5.5 | Codex review model | → Codex CLI |
