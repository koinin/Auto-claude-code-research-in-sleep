...
- **Reproducibility**: thread_id preservation allows conversation continuation
- **Meta-optimize input**: richer data for harness improvement analysis

## When to Trace

After **every** `codex exec` or `codex exec resume` call that serves a reviewer/critique function. This includes review scoring, experiment auditing, claim verification, idea critique, and patch gating.
...
After each reviewer CLI call, save the trace using the helper script:
...
bash tools/save_trace.sh \
  ...
  --purpose "<purpose>" \
  --model "<model>" \
  --thread-id "<thread_id from response>" \
