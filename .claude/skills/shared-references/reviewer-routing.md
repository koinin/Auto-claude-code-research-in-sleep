...
All review calls use **Codex CLI** (`codex exec`) with `reasoning_effort: xhigh`.

This is the default for ALL skills. No parameter, no config, no effort level changes this.

## Optional: GPT-5.4 Pro via Oracle

When the user explicitly passes `— reviewer: oracle-pro`, route the review through Oracle MCP instead of Codex CLI.
...
If not specified OR `— reviewer: codex`:
    → Use codex exec with reasoning_effort: xhigh
