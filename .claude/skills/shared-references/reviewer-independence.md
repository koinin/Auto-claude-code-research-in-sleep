...
- Any skill that sends artifacts to `codex exec` or `codex exec resume`

## Exception

Multi-round review within the SAME thread (`codex exec resume`) may reference the reviewer's own previous feedback to check resolution — but still must not include executor interpretations of that feedback.
