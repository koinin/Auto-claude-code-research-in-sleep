...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via Codex CLI for plan review, figure review, writing review, and improvement loop.
...
if paper/references.bib (or paper.bib) exists and contains entries cited from sec/*.tex:
    ...
    Fresh cross-family reviewer (gpt-5.5 via Codex CLI) with web/DBLP/arXiv lookup
    verifies each entry:
...
Each sub-audit runs in a **fresh Codex thread** (never `codex exec resume`,
