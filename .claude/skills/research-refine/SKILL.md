...
- **REVIEWER_MODEL = `gpt-5.5`** — Reviewer model used via Codex CLI.
...
| `thread_id` | string or null | Reviewer thread ID for `codex exec resume` continuity |
...
2. **On resume**, read the state file and recover context:
   ...
   - Recover `thread_id` for reviewer thread continuity
...
```bash
codex exec -m REVIEWER_MODEL -c model_reasoning_effort="xhigh" "$(cat <<'PROMPT'
...
**CRITICAL: Save the `thread_id`** from this call for all later rounds.
...
```bash
codex exec resume [saved from Phase 2] -m REVIEWER_MODEL -c model_reasoning_effort="xhigh" "$(cat <<'PROMPT'
...
- **Save `thread_id` from Phase 2** and use `codex exec resume` for later rounds.
