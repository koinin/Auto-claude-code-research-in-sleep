...
- MAX_REVIEW_ROUNDS = 3
- REVIEWER_MODEL = `gpt-5.5` via Codex CLI, reasoning effort always `xhigh`
...
**Save the thread_id.** Parse into structured issue list. Write to `PROOF_AUDIT.md`.
...
Use `codex exec resume` with saved thread_id. Include fix summaries. Request the same mandatory checklist.
...
- **Preserve thread_id**: Use `codex exec resume` for follow-up rounds.
...
`codex exec resume` across proof-checker runs. Do not accept prior audit outputs
